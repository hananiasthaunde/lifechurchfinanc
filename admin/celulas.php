...
if (isset($_GET['action']) && $_GET['action'] === 'get_available_members') {
    ...
    $stmt = $conn->prepare("SELECT id, name, email FROM users WHERE church_id = ? AND celula_id IS NULL AND (name LIKE ? OR email LIKE ?) LIMIT 20");
    if ($stmt) {
...
<body class="bg-gray-50">
                       ...
                       <div class="bg-white rounded-lg shadow-md">
                           ...
                           <div class="space-y-4 p-4">
                                ...
                                <?php if (empty($membros_celula)): ?>
                                    <div class="text-center py-4 text-gray-500">Nenhum membro nesta célula.</div>
                                <?php else: ?>
                                    <?php foreach ($membros_celula as $membro): ?>
                                    <div class="bg-white border border-gray-200 rounded-lg p-4 grid grid-cols-1 lg:grid-cols-12 lg:gap-4 lg:items-center hover:bg-gray-50 transition">
                                        <div class="col-span-4 mb-2 lg:mb-0"><span class="font-bold lg:hidden mr-2">Nome:</span><?php echo htmlspecialchars($membro['name']); ?></div>
                                        <div class="col-span-4 mb-2 lg:mb-0"><span class="font-bold lg:hidden mr-2">Email:</span><?php echo htmlspecialchars($membro['email']); ?></div>
                                        <div class="col-span-2 mb-2 lg:mb-0"><span class="font-bold lg:hidden mr-2">Telefone:</span><?php echo htmlspecialchars($membro['phone']); ?></div>
                                        <div class="col-span-2 text-left lg:text-right mt-4 lg:mt-0">
                                             <button class="remove-member-btn text-red-600 hover:text-red-800" data-id="<?php echo $membro['id']; ?>" data-name="<?php echo htmlspecialchars($membro['name']); ?>" title="Remover da Célula">
                ...
                <?php elseif ($show_create_form): ?>
                       <div class="bg-white p-6 rounded-lg shadow-md">
                           ...
                           <form method="POST" action="celulas.php" class="space-y-4">
                               ...
                               <div class="text-right"><button type="submit" class="inline-flex justify-center py-2 px-6 border border-transparent rounded-button shadow-sm font-medium text-white bg-primary hover:bg-blue-700">Criar Célula</button></div>
                           </form>
                       </div>
                       ...
                       <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
                           ...
                           <div class="space-y-4">
                                ...
                                <?php else: ?>
                                    ...
                                    <div class="bg-white border border-gray-200 rounded-lg p-4 grid grid-cols-1 md:grid-cols-4 md:gap-4 md:items-center hover:bg-gray-50 transition">
                                        <div class="col-span-1 mb-2 md:mb-0"><span class="font-bold md:hidden mr-2">Célula:</span><?php echo htmlspecialchars($item_celula['nome']); ?></div>
                                        <div class="col-span-1 mb-2 md:mb-0"><span class="font-bold md:hidden mr-2">Líder:</span><?php echo htmlspecialchars($item_celula['lider_nome']); ?></div>
                                        <div class="col-span-1 mb-2 md:mb-0"><span class="font-bold md:hidden mr-2">Igreja:</span><?php echo htmlspecialchars($item_celula['church_name']); ?></div>
                                        <div class="col-span-1 text-left md:text-right mt-2 md:mt-0"><a href="celulas.php?view_celula_id=<?php echo $item_celula['id']; ?>" class="font-medium text-primary hover:underline">Ver Detalhes</a></div>
                                    </div>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                           </div>
    ...
    <div id="selectMemberModal" class="modal fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center hidden p-4">
        <div class="modal-content bg-white rounded-lg shadow-xl w-full max-w-lg transform scale-95 opacity-0">
            <div class="flex justify-between items-center p-4 border-b">
                ...
                <button id="closeSelectMemberModal" class="text-gray-500 hover:text-gray-800"><i class="ri-close-line ri-xl"></i></button>
            </div>
            <div class="p-4">
                <input type="text" id="memberSearchInput" placeholder="Pesquisar por nome ou email..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary">
            </div>
            <div id="available-members-list" class="p-4 max-h-80 overflow-y-auto space-y-2">
